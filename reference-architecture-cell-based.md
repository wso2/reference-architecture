...
+ Asanka Abeysinghe | Deputy CTO & VP, Architecture - CTO Office | WSO2, Inc | <asankaa@wso2.com>
...
Developers implement components by writing code or configuration and test in their local sandbox environment. Once the units tests passed, they push (or pull) the changes to the shared source control repository. At that point, the CI/CD process will kickoff and deploy the components into the cells based on the cell configuration defined by the cell owner. CI/CD process takes the components and the cells through the configured application lifecycle stages. Deployment of the component can take two paths based on the immutability of the cell. Immutable cells signal the global control-plane and deploy the entire cell for a component change if the cell is mutable, CI/CD process signals the local control-plane and deploy only the updated component.
